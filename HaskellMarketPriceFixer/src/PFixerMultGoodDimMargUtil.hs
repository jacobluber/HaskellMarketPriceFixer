...
data MarketParticipant = Seller String [(String, Float, Float)] Float | Buyer String [(String, Float, (Float,Float,Float))] Float deriving (Show, Ord, Eq)
...
testbuyers = [Buyer "Z" [("a",0,(4,2,(4*100)/(0+2))),("b",0,(2,3,(2*100)/(0+3))),("c",0,(6,1,(6*100)/(0+1)))] 100, Buyer "Y" [("a",0,(5,4,(5*50)/(0+4))),("b",0,(4,2,(4*50)/(0+5))),("c",0,(2,1,(2*50)/(0+2)))] 50,Buyer "X" [("a",0,(5,1,(5*250)/(0+1))),("b",0,(3,0,(3*250)/(0+0))),("c",0,(1,2,(4*250)/(0+2)))] 250] 
...
--Buyer BuyerStringIdentifier [(GoodIdentifier,UnitsOfGoodHeld,(WealthDeccrementer,QuantityIncrementer,MarginalUtilityFunction)] MoneyHeld
...
matchMarketParticipants sellersWithInventory dudSellers activeBuyer = if sellersWithInventory == [] 
                                                                      then (activeBuyer, NoSeller, NoSeller, 0) 
                                                                            ...
                                                                            in if (buyerMoneyHeld > sellerPrice) && (mostUtility > 0) then let oldBuyerMoney = (\(Buyer a b c) -> c) activeBuyer 
                                                                                                                                               newMoney = oldBuyerMoney-sellerPrice
                                                                                                                                               relevantBuyerGoods = (\(Buyer a b c) -> b) activeBuyer
                                                                                                                                               ...
                                                                                                                                               updateBuyer = (Buyer buyerIdent newGoods newMoney)                                                                                                                                                                                                                                                                                                            
