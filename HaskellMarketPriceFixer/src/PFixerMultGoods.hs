...
data MarketParticipant = Seller [String Float Float] | Buyer [String Float Float] Float deriving (Show, Ord, Eq)
...
priceFix buyers sellers result = if buyers == [] 
                                 ...
                                 else let (Buyer l r) = head(buyers)
                                          inactiveBuyers = tail(buyers)
                                          goodsDemandedByBuyer = [x|x <- sellers, fTup(x) == fTup(l)]
